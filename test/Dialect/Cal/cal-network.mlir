...
// CHECK-LABEL: cal.network @WithParams(%a: i32, %b: f32)
// CHECK: {
// CHECK: }
...
// CHECK-LABEL: cal.network @WithInPorts(%n: i32)
// CHECK: ports_in (%inA, %inB : !fifo.output_port<i32>, !fifo.output_port<f32>)
// CHECK: {
// CHECK: }
...
// CHECK: ports_out (%outX : !fifo.input_port<i64>)
// CHECK: {
// CHECK: }
...
// CHECK-LABEL: cal.network @Full(%p0: i32)
// CHECK: ports_in (%src : !fifo.output_port<i32>)
// CHECK: ports_out (%snk : !fifo.input_port<i32>)
// CHECK: %in0, %out0 = fifo.create<i32>(2) : !fifo.input_port<i32>, !fifo.output_port<i32>
...
// CHECK-LABEL: cal.network @Parent(%limit: i32)
// CHECK: cal.create_instance @Child "child0" (%limit : i32)
// CHECK:     ports_in (%pIn : !fifo.output_port<i32>)
// CHECK:     ports_out (%pOut : !fifo.input_port<i32>)

// 7. Negative tests (these rely on diagnostic checking when run with opt expecting failure)
