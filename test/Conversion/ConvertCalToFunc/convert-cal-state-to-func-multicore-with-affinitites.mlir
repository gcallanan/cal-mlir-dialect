// RUN: cal-opt %s --convert-cal-to-func="actor-paritioning-mode=multi-threaded" | FileCheck %s
module {
  ...
  // CHECK: llvm.func @usleep(i32) -> i32
  // CHECK: func.func @src(%arg0: i32, %arg1: i32, %arg2: !fifo.input_port<i32>) -> i1
  // CHECK: func.func @sink(%arg0: !fifo.output_port<i32>) -> i1
  // CHECK: func.func @merge(%arg0: !fifo.output_port<i32>, %arg1: !fifo.output_port<i32>, %arg2: !fifo.input_port<i32>) -> i1
  ...
  cal.network {
    ...
    // Create flags used for termination detection (3 groups: CPU1, srcA, srcB)
    ...
    // CHECK-NEXT: %[[PROG_FLAGS:.*]] = memref.alloc() : memref<48xi32>
    // CHECK-NEXT: %{{.*}} = memref.atomic_rmw assign %[[C1_I32]], %[[PROG_FLAGS]][%[[C0]]] : (i32, memref<48xi32>) -> i32
    // CHECK-NEXT: %{{.*}} = memref.atomic_rmw assign %[[C1_I32]], %[[PROG_FLAGS]][%[[C16]]] : (i32, memref<48xi32>) -> i32
    // CHECK-NEXT: %{{.*}} = memref.atomic_rmw assign %[[C1_I32]], %[[PROG_FLAGS]][%[[C32]]] : (i32, memref<48xi32>) -> i32
    ...
    // CHECK-NEXT:   } do {
    // CHECK-NEXT:     %{{.*}} = func.call @merge(%[[OUTPUT_PORT]], %[[OUTPUT_PORT_1]], %[[INPUT_PORT_2]]) {device_affinity = "CPU1", from_create_instance} : (!fifo.output_port<i32>, !fifo.output_port<i32>, !fifo.input_port<i32>) -> i1
    // CHECK-NEXT:     %{{.*}} = func.call @sink(%[[OUTPUT_PORT_3]]) {device_affinity = "CPU1", from_create_instance} : (!fifo.output_port<i32>) -> i1
    // CHECK-NEXT:     %{{.*}} = arith.ori %{{.*}}, %{{.*}} : i1
    // CHECK-NEXT:     scf.if %{{.*}} {
    // CHECK-NEXT:       %{{.*}} = memref.load %[[PROG_FLAGS]][%[[C0]]] : memref<48xi32>
    // CHECK-NEXT:       %{{.*}} = arith.cmpi eq, %{{.*}}, %[[C0_I32]] : i32
    // CHECK-NEXT:       scf.if %{{.*}} {
    // CHECK-NEXT:         %{{.*}} = memref.atomic_rmw assign %[[C1_I32]], %[[PROG_FLAGS]][%[[C0]]] : (i32, memref<48xi32>) -> i32
    // CHECK-NEXT:       }
    ...
    // Group 2: srcA (no device_affinity)
    ...
    // CHECK-NEXT:     scf.if %{{.*}} {
    // CHECK-NEXT:       %{{.*}} = memref.load %[[PROG_FLAGS]][%[[C16]]] : memref<48xi32>
    // CHECK-NEXT:       %{{.*}} = arith.cmpi eq, %{{.*}}, %[[C0_I32]] : i32
    // CHECK-NEXT:       scf.if %{{.*}} {
    // CHECK-NEXT:         %{{.*}} = memref.atomic_rmw assign %[[C1_I32]], %[[PROG_FLAGS]][%[[C16]]] : (i32, memref<48xi32>) -> i32
    // CHECK-NEXT:       }
    ...
    // Group 3: srcB (no device_affinity)
    ...
    // CHECK-NEXT:     scf.if %{{.*}} {
    // CHECK-NEXT:       %{{.*}} = memref.load %[[PROG_FLAGS]][%[[C32]]] : memref<48xi32>
    // CHECK-NEXT:       %{{.*}} = arith.cmpi eq, %{{.*}}, %[[C0_I32]] : i32
    // CHECK-NEXT:       scf.if %{{.*}} {
    // CHECK-NEXT:         %{{.*}} = memref.atomic_rmw assign %[[C1_I32]], %[[PROG_FLAGS]][%[[C32]]] : (i32, memref<48xi32>) -> i32
    // CHECK-NEXT:       }
    ...
    // CHECK-NEXT: } do {
    // CHECK-NEXT:   %{{.*}} = memref.atomic_rmw assign %[[C0_I32]], %[[PROG_FLAGS]][%[[C0]]] : (i32, memref<48xi32>) -> i32
    // CHECK-NEXT:   %{{.*}} = memref.atomic_rmw assign %[[C0_I32]], %[[PROG_FLAGS]][%[[C16]]] : (i32, memref<48xi32>) -> i32
    // CHECK-NEXT:   %{{.*}} = memref.atomic_rmw assign %[[C0_I32]], %[[PROG_FLAGS]][%[[C32]]] : (i32, memref<48xi32>) -> i32
