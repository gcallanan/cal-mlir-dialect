...
for ASSIGNMENT_MODE in "round-robin" "block"; do
    ...
    for NUM_CORES in 1 2 4; do
        ...
        for RUN in $(seq 1 $NUM_TESTS); do
            START=$(date +%s%N)
            taskset -c 0-$((NUM_CORES-1)) ./streamblocks.out --cfile=streamblocks_partition_temp.xml --d=131072 2>&1 | tail -1 || true
            END=$(date +%s%N)
            sleep $SLEEP_SECS
            RUN_TIME=$(( (END - START) / 1000000 ))
            TOTAL_TIME=$(( TOTAL_TIME + RUN_TIME ))
        ...
        LINE="    NUM_CORES=$NUM_CORES: avg=${SB_TIMES[$ASSIGNMENT_MODE-$NUM_CORES]} min=${SB_MIN_TIMES[$ASSIGNMENT_MODE-$NUM_CORES]} max=${SB_MAX_TIMES[$ASSIGNMENT_MODE-$NUM_CORES]}"
        echo "$LINE"
        echo "$LINE" >> "$RESULTS_FILE"
    done
done
