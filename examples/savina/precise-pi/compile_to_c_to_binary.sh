#!/bin/bash
echo "Compiling PrecisePi network to C -> LLVM -> Binary"
...
mkdir myproject
tychoc --set experimental-network-elaboration=on --set reduction-algorithm=ordered-condition-checking --source-path PrecisePi.cal --target-path myproject precisepi.PrecisePiNetwork
