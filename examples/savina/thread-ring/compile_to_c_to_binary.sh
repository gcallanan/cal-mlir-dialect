#!/bin/bash
echo "Compiling thread ring network to C -> LLVM -> Binary"
...
mkdir myproject
tychoc --set experimental-network-elaboration=on --set reduction-algorithm=ordered-condition-checking --source-path ThreadRing.cal --target-path myproject threadRing.ThreadRing
