#!/bin/bash
echo "Compiling FilterBank network to C -> LLVM -> Binary"
...
mkdir myproject
tychoc --set experimental-network-elaboration=on --set reduction-algorithm=ordered-condition-checking --source-path FilterBank.cal --target-path myproject filterbank.FilterBankNetwork
