#!/bin/bash
echo "Compiling FilterBank network to CPP -> LLVM -> Binary"
...
streamblocks multicore --set experimental-network-elaboration=on --set reduction-algorithm=ordered-condition-checking --source-path FilterBank.cal --target-path myproject filterbank.FilterBankNetwork
...
cd ../..
cp myproject/bin/FilterBankNetwork main_executable_from_cpp
