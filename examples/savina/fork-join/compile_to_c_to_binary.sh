#!/bin/bash
echo "Compiling ForkJoin network to C -> LLVM -> Binary"
...
mkdir myproject
tychoc --set experimental-network-elaboration=on --set reduction-algorithm=ordered-condition-checking --source-path ForkJoin.cal --target-path myproject forkjoin.ForkJoinNetwork
