#!/bin/bash
echo "Compiling ForkJoin network to CPP -> LLVM -> Binary"
...
streamblocks multicore --set experimental-network-elaboration=on --set reduction-algorithm=ordered-condition-checking --source-path ForkJoin.cal --target-path myproject forkjoin.ForkJoinNetwork
...
cd ../..
cp myproject/bin/ForkJoinNetwork main_executable_from_cpp
