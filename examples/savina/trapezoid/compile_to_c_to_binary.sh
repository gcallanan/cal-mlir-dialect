#!/bin/bash
echo "Compiling trapezoid network to C -> LLVM -> Binary"
...
mkdir myproject
tychoc --set experimental-network-elaboration=on --set reduction-algorithm=ordered-condition-checking --source-path Trapezoid.cal --target-path myproject trapezoid.TrapezoidNetwork
