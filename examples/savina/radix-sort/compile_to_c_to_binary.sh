#!/bin/bash
echo "Compiling RadixSort network to C -> LLVM -> Binary"
...
mkdir myproject
tychoc --set experimental-network-elaboration=on --set reduction-algorithm=ordered-condition-checking --source-path RadixSort.cal --target-path myproject radixsort.RadixSortNetwork
