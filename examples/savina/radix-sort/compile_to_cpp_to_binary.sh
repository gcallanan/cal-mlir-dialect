#!/bin/bash
echo "Compiling RadixSort network to CPP -> LLVM -> Binary"
...
streamblocks multicore --set experimental-network-elaboration=on --set reduction-algorithm=ordered-condition-checking --source-path RadixSort.cal --target-path myproject radixsort.RadixSortNetwork
...
cd ../..
cp myproject/bin/RadixSortNetwork main_executable_from_cpp
