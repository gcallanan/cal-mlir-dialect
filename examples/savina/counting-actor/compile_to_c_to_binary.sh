#!/bin/bash
echo "Compiling CountingActor network to C -> LLVM -> Binary"
...
mkdir myproject
tychoc --set experimental-network-elaboration=on --set reduction-algorithm=ordered-condition-checking --source-path CountingActor.cal --target-path myproject countingactor.CountingActorNetwork
