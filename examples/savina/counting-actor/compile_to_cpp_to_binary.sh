#!/bin/bash
echo "Compiling CountingActor network to CPP -> LLVM -> Binary"
...
streamblocks multicore --set experimental-network-elaboration=on --set reduction-algorithm=ordered-condition-checking --source-path CountingActor.cal --target-path myproject countingactor.CountingActorNetwork
...
cd ../..
cp myproject/bin/CountingActorNetwork main_executable_from_cpp
